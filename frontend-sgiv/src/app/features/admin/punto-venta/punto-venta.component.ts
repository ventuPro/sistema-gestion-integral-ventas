...
export class PuntoVentaComponent implements OnInit {
    ...
    this.cajaService.obtenerTurnoHoy().subscribe({
      next: (res) => {
        ...
        if (this.turnoActivo?.estado_turno === 'Abierto') {
          // Tiene turno abierto → mostrar POS y cargar todo
          this.mostrarAbrirTurno = false;
          this.cargarCatalogo();
          this.cargarVentasHoy(); 
        } else {
          // No tiene turno hoy → mostrar formulario de apertura
      ...
      error: () => {
        // Si falla la verificación, mostrar formulario de apertura
        this.mostrarAbrirTurno = true;
        this.cargando = false;
        this.cdr.detectChanges();
      }
  ...
  agregarAlCarrito(producto: any) {
    ...
    if (stockDisponible <= 0) {
      alert(`Sin stock disponible para: ${producto.nombre_producto}`);
      return;
    }

    const itemEnCarrito = this.carrito.find(i => i.id_producto === producto.id_producto);

    if (itemEnCarrito) {
      itemEnCarrito.cantidad++;
      itemEnCarrito.subtotal = itemEnCarrito.cantidad * precio;
    } else {
      this.carrito.push({
        ...producto,
        cantidad:       1,
  ...
  quitarDelCarrito(index: number) {
    ...
    const prodIdx = this.productosDisponibles.findIndex(
  ...
  registrarVenta() {
    ...
    this.http.post(`${this.apiUrl}/caja/cobrar`, datosVenta, { headers }).subscribe({
      next: (res: any) => {
        ...
        this.total         = 0;
        this.cargarCatalogo();
        this.cargarVentasHoy(); // ← actualizar historial después de cada venta
        this.cargando = false;
        this.cdr.detectChanges();
      },
      error: () => {
